...
│   ├── ThumbnailContentCard.swift / AvatarContentCard.swift / ArticleContentCard.swift — ContentCard layouts, PER-TAB via `unified` (ContentCardView): Home article = name + NEWS row (NO source avatar — source carried by NEWS pill + "Read on …" link); Social = unified (CategoryPill + muted source). Team abbr chip ALWAYS bottom-left (Shared `MediaTeamBadge` on media, else on the CTA footer row), gated 2+ clubs. Avatar cards Social-only
