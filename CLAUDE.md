...
│   ├── PlayerSpotlight.swift       — one "player of the week" for Home's Module 2 ("Get to know your players"): flat fields (teamAbbreviation join key + playerName/jerseyNumber/position/watchURL). Backed by the TEMP PlayerSpotlightProvider seed; HomeViewModel rotates one per week across followed teams. Mirrors FeedItem/TeamContentItem's abbreviation-join shape.
...
│   └── PlayerSpotlightProvider.swift — ⚠️ TEMP/SCAFFOLDING: curated static seed for Home's Module 2 ("Get to know your players"). async spotlights() → [PlayerSpotlight], one real 2026-roster player per club (all 16, incl. expansion sides) with real jersey number + position — research-verified, with player-movement traps (mid-2025 transfers) screened out. watchURL → that team's real YouTube channel. Swap for a real curated/editorial source — same signature; a live source brings per-player content links + the club color for a team-colored badge.
├── Stores/
...
│   ├── PlayerSpotlightCard.swift   — Home → Module 2 ("Get to know your players") card: "PLAYER OF THE WEEK" label, a jersey-number badge, the player's name, "Position · Team", and a "Watch spotlight" link (opens watchURL). TEMP: the badge uses the app accent (Color.teamAccent(hex: nil)) since Home doesn't fetch rosters/team-color; pass the club hex for a true team-colored badge later.
...
tapping opens the channel; **(2) "Get to know your players"** — a
`PlayerSpotlightCard` ("Player of the week": jersey badge + name + Position·Team +
"Watch spotlight"), one followed-team player rotated weekly, hidden when no
followed team has a spotlight; **(3) "Play"** — a horizontal row of intentional
...
players (research-verified, expansion sides included), behind swappable async
signatures, since there's no content backend yet. Home owns no season data:
...
2. **(DONE)** ~~Teams tab + Following lens.~~ Teams directory of all 16 clubs
   ...
   - **(DONE)** ~~Your-teams-first Home — build the Home tab as a hub that leads
     ...
     **(1) "From your teams"** (real seeded team-channel content, the hook;
     ...
     - **Module 2 spotlight pipeline** — `PlayerSpotlightProvider` is a TEMP curated
       seed with a simple week-of-year rotation; the spec flags full mechanics as
       TBD (rotation logic across multiple followed teams, content sourcing,
       notification strategy). Needs a dedicated session + a real content source;
       a team-colored jersey badge needs the club hex (Home doesn't fetch rosters).
