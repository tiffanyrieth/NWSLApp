...
| Trivia per-round picks / KHG per-edition score detail | UserDefaults | retention is current + previous edition ONLY by owner rule — the server keeps the rollups (`fanzone_progress`). **⚠️ DERIVED-SYNCED for the recap window since 2026-08-13 (Gap 3):** a 2nd device RECONSTRUCTS the current+previous edition's played-set + score + picks from the user's own `quiz_answers` rows (`QuizResultsService.playedEditions` → `restorePlayedEditions`/`restorePlayedRounds`), so a round played elsewhere reads as played (score + full review) instead of a replay. No new table — it reads the write-only aggregate that already existed; older editions stay local-only (pruned). Bounded own-row read (~current+previous edition after the 35d prune) — passes 1k/100k |
