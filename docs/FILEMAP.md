...
│   ├── PlayoffBracketView.swift       — "Bracket" segment: VERTICAL round-grouped list (NEVER a horizontal tree); round headers w/ status dot (cyan upcoming / red live / green complete) + left connecting line; rows = shared PlayoffMatchupRow; Win→ card; tap→MatchDetail
│   ├── PlayoffPathView.swift          — "Your Path" segment: per-followed-team hero + accent timeline (current round→🏆), steps = shared PlayoffMatchupRow, the "Win → face X in the [round]" projection line, multi-team stacked sections + "if both win…" storyline, eliminated-team muted state
