...
4b. **⚠️ The ledger is only written by the publish path.** `markFeatured()` runs inside
   ...
   - **Repair:** `scripts/backfill_knowher_ledger.mjs <pool.json>` merges an already-published edition into
     ...
     the live pool. `--remove` inverts it, returning those players to eligibility — use it whenever an
     edition's CONTENT is being discarded and regenerated, or those players stay spent for the season on
     a quiz that no longer exists (2026-W31 precedent: generated by a downgraded model, redone with
     Opus 4.6; its 14 non-week-1 players had to be reclaimed first). Single-entry undo also exists as the
     admin `unfeature` op.
