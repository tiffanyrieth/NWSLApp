...
│   └── TriviaQuestion.swift        — one Daily-Trivia question for Home's Module 3 "Play" (game 1, per Reference/Design/games-design-spec.md): flat/Codable-shaped (id, question, exactly four `options`, `correctIndex`, a Category enum [leagueHistory/playerFacts/venues/rules/records/teamHistory, each w/ a display label] + a Difficulty enum [easy/medium/hard]). `correctAnswer` is a convenience for the results recap. Backed by the TEMP TriviaQuestionProvider seed; the daily 5 are derived in TriviaViewModel, durable stats live in TriviaStore.
├── Services/
...
│   ├── TriviaQuestionProvider.swift — ⚠️ TEMP/SCAFFOLDING: curated static seed for Home's Module 3 "Play", game 1 (Daily Trivia). async questions() → [TriviaQuestion]: 55 hand-written NWSL questions covering all 16 clubs (incl. the 2026 Denver Summit/Boston Legacy expansion sides) at mixed difficulty across all six categories. Deliberately leans on DURABLE facts (championship/league history, records, club founding, stadiums, laws of the game) and AVOIDS volatile "who currently leads in goals" trivia — a wrong quiz answer is worse than a thin category. The ViewModel serves 5/day deterministically from the whole pool; swap questions() for a real editorial/question backend (same signature) to remove.
...
│   └── TriviaStore.swift           — @Observable shared Daily-Trivia stats (Home's Module 3 "Play", game 1), persisted to UserDefaults; injected app-wide via .environment in RootTabView (like FollowingStore) so the game and the Home Play-card badge read the same state. Holds the DURABLE stats only: streak + bestStreak, lifetime totalCorrect/totalAnswered (→ `accuracy`), today's `lastScore`, and the local-day key `lastCompletedDay`. `hasPlayedToday` is the one-scored-play-per-day gate (the streak mechanic); recordCompletion(correct:outOf:) bumps the streak iff the last completion was YESTERDAY (a gap resets to 1), folds the score into lifetime accuracy, and stamps today — idempotent for the day so a re-open can't farm the streak. now/calendar injectable for deterministic tests. The transient in-session state (current question, selection) is NOT here — that's TriviaViewModel.
├── ViewModels/
...
│   ├── TriviaViewModel.swift       — @Observable; owns one Daily-Trivia SESSION (Home's Module 3 "Play", game 1). idle/loading/loaded/error State tracks the question-bank load; loadDaily() pulls TriviaQuestionProvider then locks in today's 5 via a DETERMINISTIC daily shuffle (a private SplitMix64 RNG seeded by the local day number → stable all day, rotates daily, no persistence). Holds the transient per-play state (currentIndex, selectedIndex, isRevealed, correctCount, picks[], isFinished) + the select→submit→advance/finish flow; the View commits the result to TriviaStore on finish. Durable stats are NOT here (that's TriviaStore).
...
├── Views/
│   ├── RootTabView.swift           — app root; 5-tab bottom TabView (Home / Schedule / Standings / Teams / Feed), each tab owns its own NavigationStack; LANDS ON HOME; creates the FollowingStore, the MatchStore, AND the TriviaStore and injects all three via .environment. ALL FIVE TABS ARE NOW BUILT — no placeholder tab remains (ComingSoonView is now unreferenced).
│   ├── HomeView.swift              — Home tab: the your-teams-first hub. While FollowingStore.hasOnboarded is false it renders OnboardingView in place (tab bar stays visible). Otherwise the hub: a ScrollView of modules per Reference/Design/home-tab-design-spec.md (the 2026-06-06 "content leads" reorder) — (1) "From your teams" = a vertical stack of TeamContentCards (real seeded team-channel content, newest-first, capped — THE HOOK); (2) "Get to know your players" = a vertical stack of PlayerSpotlightCards, ONE per followed team (Option B mini-profile), each a NavigationLink → PlayerSpotlightView; hidden if no followed team has a spotlight; (3) "Play" = a horizontal row of game cards under a competitive subtitle — the Daily Trivia card is now a LIVE NavigationLink → DailyTriviaView (indigo identity, a 🔥 streak badge when one's going, and a "Play now"/"Done today ✓" state line read from the injected TriviaStore), while Predict the XI / Bracket Battle stay intentional "coming soon" placeholders; (4) "Coming up" = a compact ComingUpRow per followed club (real, from MatchStore; hidden when none). ("Around the league" was REMOVED — it duplicated the Schedule tab.) Reads MatchStore + FollowingStore from the environment, hands the store to its view model, loads both once on .task. The no-follows state shows the lead module's "Choose your teams" prompt that re-presents the picker as a sheet. (Spec leftover: the per-section "See all" link is omitted until a full-content destination exists.)
│   ├── DailyTriviaView.swift       — Daily Trivia (Home's Module 3 "Play", game 1), pushed from the Home Play card so it rides Home's NavigationStack (nav-bar back is the affordance; no own stack). Five MC questions/day: select an option (changeable, indigo highlight) → Submit (reveals correct=green / your wrong pick=red, lettered A–D badges) → Next / See Results. Results screen = today's score (X/5), 🔥 streak, and all-time accuracy, plus a per-question Review recap (✓/✗ + correct answer). ONE scored play per day: when TriviaStore.hasPlayedToday it shows the locked results summary + "come back tomorrow" instead of replaying (the approved Wordle-style replay rule). Indigo "quiz" accent per the spec's distinct-but-cohesive game identity; green/red only for right/wrong reveals. Reads the shared TriviaStore (commits the session on finish); owns the in-session state via TriviaViewModel.
...
**(3) "Play"** — a horizontal row of game cards under a
competitive subtitle; the **Daily Trivia card is now live** (a `NavigationLink`
into the built game — see below), with Predict the XI / Bracket Battle still
intentional "coming soon" placeholders; **(4) "Coming up"** — a
...
**Daily Trivia (Module 3 "Play" — game 1, first built game).** Built per
`Reference/Design/games-design-spec.md` §"Game 1: Daily Trivia." The Home Play
card's Daily Trivia tile is now a live `NavigationLink` → **`DailyTriviaView`**:
five multiple-choice NWSL questions a day, select → submit (reveal correct/
incorrect immediately) → next, then a results screen with **today's score, a
streak counter, and all-time accuracy** plus a per-question review recap. **One
scored play per day** (the approved Wordle/Duolingo replay rule) — once you've
finished, re-opening shows the locked results summary and a "come back tomorrow"
prompt instead of a replay, so the streak stays meaningful. The 5 questions are
drawn from the bank by a **deterministic daily shuffle** (a SplitMix64 RNG seeded
by the local day number — stable all day, rotates as days pass, no persistence
needed). Visual identity is an **indigo "quiz" accent** (distinct from team colors
and the app's blue follow-highlight, per the spec's "own but cohesive" call);
green/red are reserved for the right/wrong reveal. Architecture follows the
established split: `TriviaQuestion` (model) + the **TEMP `TriviaQuestionProvider`
seed** (55 hand-written questions covering all 16 clubs at mixed difficulty,
leaning on *durable* facts and avoiding volatile current-season trivia) +
`TriviaViewModel` (the in-play session + daily pick) + **`TriviaStore`** (durable
streak / lifetime accuracy / day-gate, `@Observable` + UserDefaults, injected
app-wide via `.environment` in `RootTabView` like `FollowingStore`/`MatchStore`,
so the Home card badge and the game read the same state). The card itself shows a
🔥 streak badge and a "Play now" / "Done today ✓" state line off that store.
**Verified in-sim** (the project's established launch-env scaffold path — a
temporary deep-link + autoplay hook drove deterministic screenshots of the
question/reveal/results/locked screens and both Home-card states, since UI taps
flake under this machine's memory pressure, then removed): the question screen
(indigo progress + category chip + lettered A–D options), the reveal (picked
answer red ✗, correct answer green ✓), the results screen (1/5, 🔥1, 20% — the
score correctly folded into lifetime accuracy and streak), the locked
"all set for today" summary (seeded 4/5 · 🔥7 · 78%, proving the day-gate +
UserDefaults read-path), and the Home Play card in both "Play now" and
"Done today ✓ · 🔥7" states. Screenshots in the gitignored
`Reference/Design/trivia-verification/`. The Predict the XI /
Bracket Battle cards remain intentional "coming soon" placeholders (build order
per the spec).
...
2. **(DONE)** ~~Teams tab + Following lens.~~ Teams directory of all 16 clubs
   ...
   - **(DONE)** ~~Your-teams-first Home — build the Home tab as a hub that leads
     ...
     **(1) "From your teams"** (real seeded team-channel content, the hook;
     ...
     - **Module 3 "Play" (Daily Trivia DONE; other games + backend pending)** —
       **Daily Trivia is built** per `Reference/Design/games-design-spec.md`
       (`TriviaQuestion` / `TriviaQuestionProvider` / `TriviaViewModel` /
       `TriviaStore` / `DailyTriviaView`): 5 MC questions/day, select→submit→next,
       results with streak + all-time accuracy, one scored play/day, deterministic
       daily pick, indigo identity. **Still TEMP/pending:** the question bank is a
       static 55-question seed (`TriviaQuestionProvider`) — swap for a real
       editorial/question backend (same async signature) to grow the pool and add
       categories; a **leaderboard** ("rank against other players") needs the
       server/backend (#12); the spec's **share-result card** and the daily
       "keep your streak alive" **push notification** (reuses the Tier-1 local-
       notification path) are not built. **Next games** (build order per the spec):
       **Bracket Battle** (community voting tournament) then **Predict the XI**
       (matchday prediction — needs lineup data; the prediction games reuse the
       same backend as push, #12/#14).
